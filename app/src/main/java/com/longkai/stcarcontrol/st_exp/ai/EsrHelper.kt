...
import com.longkai.stcarcontrol.st_exp.Utils.mainThread
import com.longkai.stcarcontrol.st_exp.media.audio.AudioRecorder
import com.longkai.stcarcontrol.st_exp.media.audio.RecorderCallback
import java.io.File
...
class EsrHelper(
    ...
    private fun workDir(): String {
        ...
        Log.i(TAG, "longkai externalPath  " + externalPath)
//        return "${externalPath}/ai/esr"
        return "sdcard/ai/esr"
    }
...
//        //在不切换语种的情况下，如何需要重新设置命令词，需要卸载之前的命令词，再去加载新的命令词
        ...
        if (ret != AbilityConstant.ABILITY_SUCCESS_CODE) {
            ...
            mainThread {
                callBack.onAbilityError(ret, Throwable("open esr error code $ret ${workDir()}"))
            }
            return true
        }
        //指定加载设置的FSA语法文件，这里array中的index需要在上面设置的FSA语法文件中的index对应上
        //这里暂时只指定加载index == 0 和 index == 1的， index= 2
        ...
        if (ret != AbilityConstant.ABILITY_SUCCESS_CODE) {
            ...
            mainThread {
                callBack.onAbilityError(ret, Throwable("open esr specifyDataSet error $ret"))
            }
            return true
        }
        ...
        if (aiHandle?.code != AbilityConstant.ABILITY_SUCCESS_CODE) {
            ...
            mainThread {
                callBack.onAbilityError(ret, Throwable("open esr start error $ret"))
            }
            return true
        }
        return false
    ...
    override fun onResult(
        ...
        for (item in esrData) {
            ...
            if (tempKey.contains("plain") || tempKey.contains("pgs")) {
                val prefix = if (tempKey.contains("plain")) "Match: " else ""
                mainThread {
                    callBack.onAbilityResult("$prefix$tempValue")
                }
            }
        }
